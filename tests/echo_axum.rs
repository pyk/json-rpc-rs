//! Integration tests for echo_axum example.
//!
//! This test suite verifies echo_axum example works correctly by:
//! - Starting the HTTP server
//! - Sending JSON-RPC requests via HTTP POST
//! - Capturing and validating responses
//!
//! Run test:
//!
//! ```shell
//! cargo test --test echo_axum
//! ```
//!
...
mod tests {
    ...
    async fn setup_server() -> &'static str {
        ...
        let server = SERVER.get_or_init(|| {
            let binary_path = common::get_example_path("echo_axum").unwrap();
